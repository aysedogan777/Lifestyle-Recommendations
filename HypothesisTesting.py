# -*- coding: utf-8 -*-
"""
Created on Sun Jan 24 20:25:13 2021
...
#defining features and outcomes to train GAIN1
y1 = df_ori['CLASS']
...
''' GAIN 1'''
#For GAIN 1 we do not need indirectly changable variables so we are dropping them from the data
...
#the directly changable variables that has to be learned how to impute by GAIN 1
changes = [ 'BMI01', 'ETHANL03', 'SMKSTA', 'MOVE', 'CARB', 'CHOL', 
        'DFIB','PROT', 'SFAT', 'TFAT', 'TOTCAL03',]
#these variables must be defined as missing values 
for col in changes:
...
#same training data is going to be used 
df2 = X_forTrain.copy()


#the indirectly changable features must be learned by GAIN 2 and imputed 
changes2 = [ 'HDLSIU02', 'LDLSIU02', 'TCHSIU01','TRGSIU01', 'SBPA21', 'SBPA22', 'ANTA07A', 'ANTA07B', 'ECGMA31',
       'HMTA03','APASIU01', 'APBSIU01', 'LIPA08', 'CHMA09']
#these variables are defined as missing values 
for col in changes2:
...
#finally train the classifier with balanced data
...
#lifestyle features that needs to be imputed by the model to get lifestyle recommendations
#Directly changable variables:
ind_dir_feat = ['BMI01', 'ETHANL03', 'SMKSTA', 'MOVE', 'CARB', 'CHOL', 'DFIB',
       'PROT', 'SFAT', 'TFAT', 'TOTCAL03','CLASS']

#Defining objects before the loop

New_R1  = list()#the risk of the recommended lifestyle will be kept in here
New_R0  = list()#the risk of the original lifestyle will be kept in here
New_R0L = list()
New_R1L = list()

# the lists  for GAIN algprithms
min_risk = list()
max_risk = list()
...
for z in range(test2.shape[0]):#test2.shape[0]
    ...
    ori = pd.DataFrame() #original records of the patient
    ...
    if ori.iloc[:,-1].item() == 1:
        ...
        #utility for the original lifestyle
        ...
        #patinet's original data with the calculated values
        values = pd.DataFrame(new_row).T 
        values.columns = cols
        new1 = pd.DataFrame()
        new_large =  pd.DataFrame()
        
        #we do not need indirectly changable featuress in the lifestyle generations. Thus, indirectly changable features columns are dropped from the data set.
        ori_direct = ori_direct.drop(columns=['HDLSIU02', 'LDLSIU02', 'TCHSIU01','TRGSIU01', 'SBPA21', 'SBPA22', 'ANTA07A', 'ANTA07B', 'ECGMA31',
           'HMTA03','APASIU01', 'APBSIU01', 'LIPA08', 'CHMA09'], axis = 1)

        test = pat.copy()
        
        '''directly changable features''' # are dropped from the data set and defined as missing values 
        for col in ind_dir_feat:
            ori_direct.loc[ori_direct.sample(frac=1).index, col] = pd.np.nan
            
        #500 replications are prepared to impute indirectly changable features
        ...
        df.columns = columns 
        #filled directly changable features are predicted by GAIN and saved in csv file:
        df.to_csv('Recs.csv')
    
        X_new = pd.DataFrame()  
        X_new = X_new.append(df.head(0), ignore_index = False)
        
        #Alternative lifestyle recommendations are going to be filtered based on the literature
        a = 0
        for i in range(_):
            if ori['BMI01'].mean() <= 2:
                                ...
                                if ori['SFAT'].mean() <= 3:
                                    ...
                                    if ori['TFAT'].mean() <= 4:
                                        ...
                                        if df.iloc[i, 0] <= ori['BMI01'].mean() and df.iloc[i, 1]  <= ori['ETHANL03'].mean() and df.iloc[i, 2] < 1 and df.iloc[i,3] >= ori['MOVE'].mean() and df.iloc[i,5] <= ori['CHOL'].mean() and df.iloc[i,6] >= ori['DFIB'].mean()  and df.iloc[i,8] <= 3 and df.iloc[i,9] <= ori['TFAT'].mean():
                                            ...
                                            X_new = X_new.append(lifestyle)
                                            #print('Constraint64b')   
        #delete the same lifetsyle recommendations just select the unique ones 
        if X_new.shape[0] == 1:
            X_new1 = X_new
        else:
        ...
        #indirectly changable variables are added to the column names for imputation
        def listmaker(n,i):
            listofzeros = [i] * n
            return listofzeros
        ...
        p_list = list()
        t_list = list()    
    #IC predictions for Recommended lifestyles 
        for k in range(X_new1.shape[0]):
            ...
            columns = [	"BMI01",	"ETHANL03",	"SMKSTA",	"MOVE",	"CARB",	"CHOL",	"DFIB",	"PROT",	"SFAT",	
                   ...
                   "HYPTMDCODE01",	"HYPERT04",	"CIGTYR01",
                   "V1AGE01",	"RACEGRP",	"DIABTS02",	"ELEVEL01",	"GENDER",	"CIGT01",	"ANTA01",	"CHOLMDCODE01",	"CLASS"]
            #lifetyle with the predicted indirectly changable features
            df.columns = columns 
            ...
            #calculating the the CVD risks for the lifetyle with predicted indirectly changable features
            ...
            #Calculate the utility values after the lifestyle generations and indirectly changable feature predictions
        ...
        if len(new1) > 0:
            ...
            #order the utility values of lifetyle from larger to smaller
            new1 = new1.sort_values(by='utility',ascending=False)
            new2 = new1.copy()
            
            #add original lifetyle to the top of the list
            frames = [values,new2]
            ...
            discrete.index = row
            #create a list for original risk of the test patients
